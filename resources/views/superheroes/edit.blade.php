...
<body>
    <nav class="navbar navbar-dark bg-dark mb-4">
        <div class="container">
            <span class="navbar-brand mb-0 h1">✏️ Editar Superhéroe</span>
        </div>
    </nav>
            ...
            <div class="card-body">
                <form action="{{ route('superheroes.update', $superhero) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $superhero->name) }}" required>
                        @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Nombre Real</label>
                        <input type="text" name="real_name" class="form-control @error('real_name') is-invalid @enderror" value="{{ old('real_name', $superhero->real_name) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="gender" class="form-control @error('gender') is-invalid @enderror" required>
                            <option value="Male" {{ old('gender', $superhero->gender) == 'Male' ? 'selected' : '' }}>Masculino</option>
                            <option value="Female" {{ old('gender', $superhero->gender) == 'Female' ? 'selected' : '' }}>Femenino</option>
                            <option value="Other" {{ old('gender', $superhero->gender) == 'Other' ? 'selected' : '' }}>Otro</option>
                        </select>
